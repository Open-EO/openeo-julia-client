# This file was generated by the Julia OpenAPI Code Generator
# Do not modify this file directly. Modify the OpenAPI specification instead.


@doc raw"""Capabilities

    Capabilities(;
        api_version=nothing,
        backend_version=nothing,
        stac_version=nothing,
        type=nothing,
        id=nothing,
        title=nothing,
        description=nothing,
        conformsTo=nothing,
        production=false,
        endpoints=nothing,
        billing=nothing,
        links=nothing,
    )

    - api_version::String : Version number of the openEO specification this back-end implements.
    - backend_version::String : Version number of the back-end implementation. Every change on back-end side MUST cause a change of the version number.
    - stac_version::String : The [version of the STAC specification](https://github.com/radiantearth/stac-spec/releases), which MAY not be equal to the [STAC API version](#tag/EO-Data-Discovery/STAC). Supports versions 0.9.x and 1.x.x.
    - type::String : For STAC versions &gt;&#x3D; 1.0.0-rc.1 this field is required.
    - id::String : Identifier for the service. This field originates from STAC and is used as unique identifier for the STAC catalog available at &#x60;/collections&#x60;.
    - title::String : The name of the service.
    - description::String : A description of the service, which allows the service provider to introduce the user to its service. [CommonMark 0.29](http://commonmark.org/) syntax MAY be used for rich text representation.
    - conformsTo::Vector{String} : Lists all conformance classes specified in various standards that the implementation conforms to. Conformance classes are commonly used in all OGC APIs and the STAC API specification.  The general openEO conformance class is &#x60;https://api.openeo.org/1.2.0&#x60;. See the individual openEO API extensions for their conformance classes.  This property is REQUIRED for STAC API versions 1.0.0-beta.1 and later.
    - production::Bool : Specifies whether the implementation is ready to be used in production use (&#x60;true&#x60;) or not (&#x60;false&#x60;). Clients SHOULD only connect to non-production implementations if the user explicitly confirmed to use a non-production implementation. This flag is part of &#x60;GET /.well-known/openeo&#x60; and &#x60;GET /&#x60;. It MUST be used consistently in both endpoints.
    - endpoints::Vector{Endpoint} : Lists all supported endpoints. Supported are all endpoints, which are implemented, return usually a 2XX or 3XX HTTP status code and are fully compatible to the API specification. An entry for this endpoint (path &#x60;/&#x60; with method &#x60;GET&#x60;) SHOULD NOT be listed. Each path MUST only be listed once in the array.
    - billing::Billing
    - links::Vector{Link} : Links related to this service, e.g. the homepage of the service provider or the terms of service.  It is highly RECOMMENDED to provide links with the following &#x60;rel&#x60; (relation) types:  1. &#x60;version-history&#x60;: A link back to the Well-Known URL (including &#x60;/.well-known/openeo&#x60;, see the corresponding endpoint for details) to allow clients to work on the most recent version.  2. &#x60;terms-of-service&#x60;: A link to the terms of service. If a back-end provides a link to the terms of service, the clients MUST provide a way to read the terms of service and only connect to the back-end after the user agreed to them. The user interface MUST be designed in a way that the terms of service are not agreed to by default, i.e. the user MUST explicitly agree to them.  3. &#x60;privacy-policy&#x60;: A link to the privacy policy (GDPR). If a back-end provides a link to a privacy policy, the clients MUST provide a way to read the privacy policy and only connect to the back-end after the user agreed to them. The user interface MUST be designed in a way that the privacy policy is not agreed to by default, i.e. the user MUST explicitly agree to them.  4. &#x60;service-desc&#x60; or &#x60;service-doc&#x60;: A link to the API definition. Use &#x60;service-desc&#x60; for machine-readable API definition and  &#x60;service-doc&#x60; for human-readable API definition. Required if full OGC API compatibility is desired.  5. &#x60;conformance&#x60;: A link to the Conformance declaration (see &#x60;/conformance&#x60;).  Required if full OGC API compatibility is desired.  6. &#x60;data&#x60;: A link to the collections (see &#x60;/collections&#x60;). Required if full OGC API compatibility is desired.  7. &#x60;create-form&#x60;: A link to a user registration page.  8. &#x60;recovery-form&#x60;: A link to a page where a user can recover a user account (e.g. to reset the password or send a reminder about the username to the user&#39;s email account).  For additional relation types see also the lists of [common relation types in openEO](#section/API-Principles/Web-Linking).
"""
Base.@kwdef mutable struct Capabilities <: OpenAPI.APIModel
    api_version::Union{Nothing, String} = nothing
    backend_version::Union{Nothing, String} = nothing
    stac_version::Union{Nothing, String} = nothing
    type::Union{Nothing, String} = nothing
    id::Union{Nothing, String} = nothing
    title::Union{Nothing, String} = nothing
    description::Union{Nothing, String} = nothing
    conformsTo::Union{Nothing, Vector{String}} = nothing
    production::Union{Nothing, Bool} = false
    endpoints::Union{Nothing, Vector} = nothing # spec type: Union{ Nothing, Vector{Endpoint} }
    billing = nothing # spec type: Union{ Nothing, Billing }
    links::Union{Nothing, Vector} = nothing # spec type: Union{ Nothing, Vector{Link} }

    function Capabilities(api_version, backend_version, stac_version, type, id, title, description, conformsTo, production, endpoints, billing, links, )
        OpenAPI.validate_property(Capabilities, Symbol("api_version"), api_version)
        OpenAPI.validate_property(Capabilities, Symbol("backend_version"), backend_version)
        OpenAPI.validate_property(Capabilities, Symbol("stac_version"), stac_version)
        OpenAPI.validate_property(Capabilities, Symbol("type"), type)
        OpenAPI.validate_property(Capabilities, Symbol("id"), id)
        OpenAPI.validate_property(Capabilities, Symbol("title"), title)
        OpenAPI.validate_property(Capabilities, Symbol("description"), description)
        OpenAPI.validate_property(Capabilities, Symbol("conformsTo"), conformsTo)
        OpenAPI.validate_property(Capabilities, Symbol("production"), production)
        OpenAPI.validate_property(Capabilities, Symbol("endpoints"), endpoints)
        OpenAPI.validate_property(Capabilities, Symbol("billing"), billing)
        OpenAPI.validate_property(Capabilities, Symbol("links"), links)
        return new(api_version, backend_version, stac_version, type, id, title, description, conformsTo, production, endpoints, billing, links, )
    end
end # type Capabilities

const _property_types_Capabilities = Dict{Symbol,String}(Symbol("api_version")=>"String", Symbol("backend_version")=>"String", Symbol("stac_version")=>"String", Symbol("type")=>"String", Symbol("id")=>"String", Symbol("title")=>"String", Symbol("description")=>"String", Symbol("conformsTo")=>"Vector{String}", Symbol("production")=>"Bool", Symbol("endpoints")=>"Vector{Endpoint}", Symbol("billing")=>"Billing", Symbol("links")=>"Vector{Link}", )
OpenAPI.property_type(::Type{ Capabilities }, name::Symbol) = Union{Nothing,eval(Base.Meta.parse(_property_types_Capabilities[name]))}

function check_required(o::Capabilities)
    o.api_version === nothing && (return false)
    o.backend_version === nothing && (return false)
    o.stac_version === nothing && (return false)
    o.id === nothing && (return false)
    o.title === nothing && (return false)
    o.description === nothing && (return false)
    o.endpoints === nothing && (return false)
    o.links === nothing && (return false)
    true
end

function OpenAPI.validate_property(::Type{ Capabilities }, name::Symbol, val)
    if name === Symbol("api_version")
        OpenAPI.validate_param(name, "Capabilities", :enum, val, ["1.2.0"])
    end
    if name === Symbol("stac_version")
        OpenAPI.validate_param(name, "Capabilities", :pattern, val, r"^(0\.9.\d+|1\.\d+.\d+)")
    end
    if name === Symbol("type")
        OpenAPI.validate_param(name, "Capabilities", :enum, val, ["Catalog"])
    end
    if name === Symbol("description")
        OpenAPI.validate_param(name, "Capabilities", :format, val, "commonmark")
    end
end
