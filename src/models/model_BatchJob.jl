...
Base.@kwdef mutable struct BatchJob <: OpenAPI.APIModel
    ...
    function BatchJob(id, title, description, process, status, progress, created, updated, plan, costs, budget, usage, log_level, links, )
        OpenAPI.validate_property(BatchJob, Symbol("id"), id)
        OpenAPI.validate_property(BatchJob, Symbol("title"), title)
        OpenAPI.validate_property(BatchJob, Symbol("description"), description)
        OpenAPI.validate_property(BatchJob, Symbol("process"), process)
        OpenAPI.validate_property(BatchJob, Symbol("status"), status)
        OpenAPI.validate_property(BatchJob, Symbol("progress"), progress)
        OpenAPI.validate_property(BatchJob, Symbol("created"), created)
        OpenAPI.validate_property(BatchJob, Symbol("updated"), updated)
        OpenAPI.validate_property(BatchJob, Symbol("plan"), plan)
        OpenAPI.validate_property(BatchJob, Symbol("costs"), costs)
        OpenAPI.validate_property(BatchJob, Symbol("budget"), budget)
        OpenAPI.validate_property(BatchJob, Symbol("usage"), usage)
        OpenAPI.validate_property(BatchJob, Symbol("log_level"), log_level)
        OpenAPI.validate_property(BatchJob, Symbol("links"), links)
        return new(id, title, description, process, status, progress, created, updated, plan, costs, budget, usage, log_level, links, )
    end
end # type BatchJob

const _property_types_BatchJob = Dict{Symbol,String}(Symbol("id")=>"String", Symbol("title")=>"String", Symbol("description")=>"String", Symbol("process")=>"ProcessGraphWithMetadata", Symbol("status")=>"String", Symbol("progress")=>"Float64", Symbol("created")=>"ZonedDateTime", Symbol("updated")=>"ZonedDateTime", Symbol("plan")=>"String", Symbol("costs")=>"Float64", Symbol("budget")=>"Float64", Symbol("usage")=>"BatchJobUsage", Symbol("log_level")=>"MinLogLevelDefault", Symbol("links")=>"Vector{Link}", )
...
function OpenAPI.validate_property(::Type{ BatchJob }, name::Symbol, val)
    if name === Symbol("id")
        OpenAPI.validate_param(name, "BatchJob", :pattern, val, r"^[\w\-\.~]+$")
    end
    if name === Symbol("description")
        OpenAPI.validate_param(name, "BatchJob", :format, val, "commonmark")
    end
    if name === Symbol("status")
        OpenAPI.validate_param(name, "BatchJob", :enum, val, ["created", "queued", "running", "canceled", "finished", "error"])
    end
    if name === Symbol("progress")
        OpenAPI.validate_param(name, "BatchJob", :maximum, val, 100, false)
        OpenAPI.validate_param(name, "BatchJob", :minimum, val, 0, false)
    end
    if name === Symbol("created")
        OpenAPI.validate_param(name, "BatchJob", :format, val, "date-time")
    end
