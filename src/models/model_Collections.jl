...
"""
Base.@kwdef mutable struct Collections <: OpenAPI.APIModel
    collections::Union{Nothing, Vector} = nothing # spec type: Union{ Nothing, Vector{Collection} }
    links::Union{Nothing, Vector} = nothing # spec type: Union{ Nothing, Vector{Link} }

    function Collections(collections, links, )
        OpenAPI.validate_property(Collections, Symbol("collections"), collections)
        OpenAPI.validate_property(Collections, Symbol("links"), links)
        return new(collections, links, )
    end
end # type Collections

const _property_types_Collections = Dict{Symbol,String}(Symbol("collections")=>"Vector{Collection}", Symbol("links")=>"Vector{Link}", )
OpenAPI.property_type(::Type{ Collections }, name::Symbol) = Union{Nothing,eval(Base.Meta.parse(_property_types_Collections[name]))}

function check_required(o::Collections)
    o.collections === nothing && (return false)
    o.links === nothing && (return false)
    true
end

function OpenAPI.validate_property(::Type{ Collections }, name::Symbol, val)
end
